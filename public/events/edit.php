...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($title === '' || $location === '' || $description === '' || $date_part === '' || $time_part === '' || !$organisation_id) {
    ...
    $event->date = $date_part . ' ' . $time_part . ':00';
    $event->organisation_id = $organisation_id;

    if ($event->update()) {
      VolunteerEvent::resetAssignments($event->event_id, $volunteers);
      header("Location: view.php?id={$event->event_id}&success=1");
      exit;
    } else {
...
<head>
  ...
  <title>Edit Event · CommunityLink</title>
        ...
        <div class="card-body">
          ...
          <form method="post">
            ...
            <div class="mb-3">
              ...
              <select id="organisation_id" name="organisation_id" class="form-select" required>
                ...
                <?php foreach ($orgs as $o): ?>
                  <option value="<?= $o['organisation_id'] ?>" <?= $o['organisation_id'] == $event->organisation_id ? 'selected' : '' ?>>
                    <?= htmlspecialchars($o['organisation_name']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
            </div>
              ...
              <select id="volunteers" name="volunteers[]" class="form-select" multiple>
                <?php foreach ($allVolunteers as $v): ?>
                  <option value="<?= $v['volunteer_id'] ?>"
                    <?= in_array($v['volunteer_id'], $assignedVolunteers) ? 'selected' : '' ?>>
            ...
            <div class="d-flex gap-2">
              ...
              <a href="view.php?id=<?= $event->event_id ?>" class="btn btn-outline-secondary">Cancel</a>
            </div>
          </form>
