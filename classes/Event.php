...
class Event extends Model
{
    ...
    {
        $st = self::$pdo->prepare("SELECT * FROM Events WHERE event_id=?");
        $st->execute([$id]);
        $r = $st->fetch();
    ...
    {
        $st = self::$pdo->prepare("SELECT * FROM Events WHERE date >= CURDATE() ORDER BY date ASC");
        $st->execute();
    ...
    {
        $st = self::$pdo->prepare("DELETE FROM Events WHERE event_id=?");
